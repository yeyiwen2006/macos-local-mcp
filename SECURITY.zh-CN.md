...
可选的 Seatbelt（sandbox-exec）配置可缩小影响面：`workspace-write` 只允许写工作目录和系统临时目录并禁止网络；`read-only` 禁止一切写入和网络。Seatbelt 不是硬安全边界；`sandbox=None` 保持当前用户原始权限。所有模式下，凭据形态的环境变量（GitHub/AWS/Anthropic/Google/Stripe 令牌、SSH agent、Tunnel/API 密钥）都不会传给子进程。
