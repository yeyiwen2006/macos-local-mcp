...
## 0.3.0 文件编辑与搜索（2026-09-26）

Windows / Python 3.13.5 独立环境的便携回归通过 **115 项测试，33 项跳过**。跳过项包括 macOS 原生命令、桌面探测、ACL/扩展属性元数据和 MCP 服务启动，以及本机无权限创建的符号链接。macOS 原生结果以本次 PR 的 Actions 为准，便携结果不能替代。

修复原生扩展属性检查后的 [GitHub Actions](https://github.com/yeyiwen2006/macos-local-mcp/actions/runs/36217850889) 已通过。macOS 15 / Apple Silicon / Python 3.13.15 为 **148 项通过，无跳过**；Windows portable 为 **117 项通过、31 项跳过**。原生用例实际创建 ACL、扩展属性和 resource fork，确认拒绝覆盖后正文及元数据仍保留，同时验证普通文件的执行位和 MCP 编辑流程。

新增用例覆盖唯一精确匹配与重叠歧义、中文/emoji、UTF-8 BOM、UTF-16 端序、GB18030、换行、版本冲突、备份失败、外部修改、暂停、受保护路径、搜索的结果/字节/条目/时间限制，以及审计不含正文。真实 MCP stdio 用例完成工具发现、搜索、读取、编辑、备份、过期版本拒绝和暂停。macOS 的该用例只在原生 runner 运行。

已完成独立静态代码审查。测试使用独立工作副本和临时文件，没有升级正在运行的服务。工具约定见[文件编辑与搜索](docs/file-tools.md)。
